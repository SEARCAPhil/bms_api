...
if($method=="POST"){
	...
	if ($id) {
		...
		if ($res[0]) {
			...
			$isChanged = $Index->change_signatories($id, $orig_recommended_by, $orig_recommended_by_position, $orig_requested_by, $orig_requested_by_position, $orig_approved_by, $orig_approved_by_position);
			# log
			if ($isChanged) {
				$payload = [
					'id' => $id,
					'orig_recommended_by' => $orig_recommended_by,
					'orig_recommended_by_position' => $orig_recommended_by_position,
					'orig_requested_by' => $orig_requested_by,
					'orig_requested_by_position' => $orig_requested_by_position,
					'orig_approved_by' => $orig_approved_by,
					'orig_approved_by_position' => $orig_approved_by_position
				];
				
				$Logs->log($current_session[0]->account_id, 'signatory', 'bidding', $id, json_encode($payload));
			}
			echo $isChanged;
		}
	}
